...
export const SignUpForm: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (trimmedUsername.length < 2) {
            setError(AUTH_CONTENT.errors.usernameLength);
            return;
        }
        ...
        if (password.length < 6) {
            setError(AUTH_CONTENT.errors.weakPassword);
            return;
        }

        setIsSubmitting(true);
        try {
            await firebaseAuthService.signUp({ email: normalizedEmail, password, username: trimmedUsername });
            setSuccess(content.successMessage);
        } catch (err: any) {
            const code = err.code;
            if (code === 'auth/email-already-in-use') {
                setError(AUTH_CONTENT.errors.emailInUse);
            } else if (code === 'auth/invalid-email') {
                setError(AUTH_CONTENT.errors.invalidEmail);
            } else if (code === 'auth/weak-password') {
                setError(AUTH_CONTENT.errors.weakPassword);
            } else if (code === 'auth/operation-not-allowed') {
                setError(AUTH_CONTENT.errors.operationNotAllowed);
        ...
        <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-2xl shadow-xl border border-gray-100">
            ...
            <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                <div className="space-y-4">
                    <div>
                        <label htmlFor="username" className="block text-sm font-bold text-gray-700 mb-1">{content.usernameLabel}</label>
                        <input
                            id="username"
                            name="username"
                            type="text"
                ...
                <div className="text-center mt-4">
                    <p className="text-sm text-gray-600">
                        ...
                        <Link to={ROUTES.LOGIN} className="font-bold text-mcd-red hover:text-red-700 transition-colors">
                            {content.loginLink}
                        </Link>
                    </p>
                </div>
